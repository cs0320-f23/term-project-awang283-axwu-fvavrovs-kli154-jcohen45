...
import org.springframework.web.multipart.MultipartFile;
  ...
  public class PosterController {
        ...
        return postersFuture
            .thenApply(
                posters ->
                    posters.stream()
                        .sorted(
                            Comparator.comparing(Poster::getCreatedAt)) // Sort by createdAt date
     ...
     * JSON object that contains some old data mixed with new data (since users will likely only
     * want to update 1 field at a time)
     *
     * @param id            the id of the poster to be updated
     * @param updatedPoster the new poster filled in with fields that you want (see fields expected
     *                      in Poster class) expected in JSON format in the request body
     * @return instance that contains a "message" (string) field and, if successful (the id is
     * found), a "data" (JSON) field that contains the data of the poster that was just deleted
     */
    ...
    public CompletableFuture<List<Poster>> sortBySoonest(
        CompletableFuture<List<Poster>> myPosters) {
      ...
      CompletableFuture<List<Poster>> afterPosters = posterService.getPosters().thenApply(
          posters -> posters.stream()
              ...
              .collect(Collectors.toList()));
      CompletableFuture<List<Poster>> allPosters = afterPosters.thenCombine(beforePosters,
          (list1, list2) -> {
            list1.addAll(list2);
            return list1;
          });
